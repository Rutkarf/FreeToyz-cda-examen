...
public class ToyzController {
    ...
    public ResponseEntity<String> add( @RequestParam("name") String name,
        ...
        if(!picture.isEmpty()){
            if (toyzService.uploadPicture(picture)){
                toyzService.add(new ToyzServiceModel(name, description, cotation, quantite, picture));
            };
        }
        return new ResponseEntity<>("Le Toyz " + name +" a été ajoutée",HttpStatus.OK) ;
    }
    ...
    public ArrayList<ToyzGetDTO> findAll(ArrayList<ToyzGetDTO> ToyzGetDTO){
       ...
       toyzServiceModels.forEach((item)->ToyzDTO.add(new ToyzServiceModel(item.getId().get(), item.getName(), item.getDescription(), item.getCotation(), item.getQuantite(), item.getPicture())) );
    ...
    public ResponseEntity<ToyzGetDTO> findById(@PathVariable Long id){
        try{
            ...
            return new ResponseEntity<>(new ToyzGetDTO ( toyzServiceModel.getId().get(), toyzServiceModel.getName(), toyzServiceModel.getDescription(), toyzServiceModel.getCotation(), toyzServiceModel.getQuantite(), toyzServiceModel.getPicture()), HttpStatus.OK) ;
